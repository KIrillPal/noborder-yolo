...
import json
TEST_NAME = 'dense'
with open(f'/alpha/projects/wastie/code/kondrashov/delta/groups/gt_24_12_dense_c06.pkl', 'rb') as f:
    gt_extended_groups = pickle.load(f)
with open(f'/alpha/projects/wastie/code/kondrashov/delta/groups/pred_24_12_dense_c06.pkl', 'rb') as f:
...
def process_combination(args, gt, pred_groups, N_CLASSES, IMAGE_SHAPE, IOU_THRESHOLD):
    ...
    return (("box", agg_type, remove_border, float(input_conf), threshold, float(max_conf)), (f1, precision, recall))
...
def parallel_parameter_search(gt, pred_groups, N_CLASSES, IMAGE_SHAPE, IOU_THRESHOLD):
    ...
    param_combinations = [
        ...
        for agg_type in ["nms", "soft-nms", "union"]
        if max_conf >= input_conf
    ]
...
if __name__ == '__main__':
    ...
    print(results)
    with open(f'box_{TEST_NAME}_gridsearch.pkl', 'wb') as f:
