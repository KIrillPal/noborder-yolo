...
gt_groups = [g for g in gt_groups if max([max(list(get_box(o))) for o in g]) < 150000]
pred_groups = [g for g in pred_groups if max([max(list(get_box(o))) for o in g]) < 150000]
...
def process_combination(args, gt, pred_groups, N_CLASSES, IMAGE_SHAPE, IOU_THRESHOLD):
    ...
    return ((("box" if USE_BOX else "mask"), agg_type, remove_border, float(input_conf), threshold, float(max_conf)), (f1, precision, recall))
...
if __name__ == '__main__':
    ...
    print(results)
    with open(f'{"box" if USE_BOX else "mask"}_val_gridsearch.pkl', 'wb') as f:
