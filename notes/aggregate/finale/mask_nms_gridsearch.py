...
def process_combination(args, gt, pred_groups, N_CLASSES, IMAGE_SHAPE, IOU_THRESHOLD):
    remove_border, input_conf, threshold, max_conf = args
    ...
    aggregator = Aggregator(classifier, NMSMerger(N_CLASSES, IOU_THRESHOLD))
    ...
    return (("mask", "nms", remove_border, float(input_conf), threshold, float(max_conf)), (f1, precision, recall))
...
def parallel_parameter_search(gt, pred_groups, N_CLASSES, IMAGE_SHAPE, IOU_THRESHOLD):
    ...
    param_combinations = [
        (remove_border, input_conf, threshold, max_conf)
...
if __name__ == '__main__':
    ...
    print(results)
    with open(f'box_nms_gridsearch.pkl', 'wb') as f:
...
# pred_merger = UnionMerger(N_CLASSES)
...
#                     confidence_classifier
#                 ])
#                 aggregator = Aggregator(classifier, pred_merger)
#                 pred = aggregator.aggregate(pred_groups)
...
#                 results[("mask", "union", remove_border, float(input_conf), threshold, float(max_conf))] = (f1, precision, recall)
#                 print(("mask", "union", remove_border, float(input_conf), threshold, float(max_conf)), "=>", (f1, precision, recall))
